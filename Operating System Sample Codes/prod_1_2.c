...
int main()
{
  ...
  // as discussed in ipc.txt/ipcs_2.txt,for more details 
  //refer to ipc.txt/ipcs_2.txt  
  ...
  // semaphore will be the max no. of free slots, in the 
  // IPC object - in this context, it is 50  
  //
  ...
  // refe to ipc.txt - this has all the characteristics 
 ...
 //object !!! 
  ...
  shma->used_slot_count = 0; 
   ...
   while(1)
   {
    ...
    //initial value of the first counting semaphore 
    ...
    //if semaphore value is +ve, it means, that there
    //are free slots - before using a free slot, we 
    //need to decrement the semaphore value by 1
    //if there are no free slots, the current semaphore 
    //value will be 0 - if the free slots drop to 0, 
    //this semop() will block the producer process
